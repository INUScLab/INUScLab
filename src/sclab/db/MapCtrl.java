...
public class MapCtrl {
	...
	public ArrayList<User> getOverUsedDongList() {
		...
		String sql = "SELECT DISTINCT UMDONG FROM USER where code in (SELECT DISTINCT code FROM CONSUMPTION WHERE overused=1 and DATE='2015-03-01')";
	...
	public ArrayList<User> getNormalDongList() {
		...
		String sql = "SELECT DISTINCT UMDONG FROM USER where code in (SELECT DISTINCT code FROM CONSUMPTION WHERE overused=0 and DATE='2015-03-01')";
